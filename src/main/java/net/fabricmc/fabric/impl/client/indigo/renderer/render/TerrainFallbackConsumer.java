 ...
 */
public class TerrainFallbackConsumer extends AbstractQuadRenderer implements Consumer<IBakedModel> {
	...
	@Override
	public void accept(IBakedModel model) {
		final Supplier<Random> random = blockInfo.randomSupplier;
		final Value defaultMaterial = blockInfo.defaultAo && model.isAmbientOcclusion()
				? MATERIAL_SHADED : MATERIAL_FLAT;
		final IBlockState blockState = blockInfo.blockState;

		for (int i = 0; i < 6; i++) {
			EnumFacing face = ModelHelper.faceFromIndex(i);
			List<BakedQuad> quads = model.getQuads(blockState, face, random.get().nextLong());
		...
		List<BakedQuad> quads = model.getQuads(blockState, null, random.get().nextLong());
		final int count = quads.size();
	...
	private void renderQuad(BakedQuad quad, EnumFacing cullFace, Value defaultMaterial) {
		...
		final EnumFacing lightFace = quad.getFace();
		editorQuad.lightFace(lightFace);
		editorQuad.nominalFace(lightFace);
		editorQuad.colorIndex(quad.getTintIndex());
