...
public abstract class AbstractQuadRenderer {
    ...
    protected void tesselateSmoothEmissive(MutableQuadViewImpl q, int renderLayer, int blockColorIndex) {
        ...
        for (int i = 0; i < 4; i++) {
            q.spriteColor(i, 0, ColorHelper.multiplyRGB(q.spriteColor(i, 0), aoCalc.ao[i]));
            q.lightmap(i, 61680); // F0 F0 = 15*16 15*16
        }
    ...
    int flatBrightness(MutableQuadViewImpl quad, IBlockState blockState, BlockPos pos) {
        mpos.setPos(pos);
        ...
        if (quad.cullFace() != null) {
            ...
            if ((flags & LIGHT_FACE_FLAG) != 0 || ((flags & AXIS_ALIGNED_FLAG) != 0 && blockState.isFullCube())) {
                mpos.move(quad.lightFace());
            }
        }

        return blockInfo.blockState.getPackedLightmapCoords(blockInfo.blockView, mpos);
