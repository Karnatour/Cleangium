...
class AoFaceData {
	...
	void l2(int l2) {
		...
		int blockLight2 = 0;
		for (int shift = 24; shift >= 0; shift -= 8) {
			int currentByte = (l2 >> shift) & 0xFF;
			if (currentByte != 0) {
				skyLight2 = currentByte;
				int mask = (1 << shift) - 1;
				blockLight2 = l2 & mask;
				break;
			}
	...
	void l3(int l3) {
		...
		int blockLight3 = 0;
		for (int shift = 24; shift >= 0; shift -= 8) {
			int currentByte = (l3 >> shift) & 0xFF;
			if (currentByte != 0) {
				skyLight3 = currentByte;
				int mask = (1 << shift) - 1;
				blockLight3 = l3 & mask;
				break;
			}
