...
public abstract class AbstractMeshConsumer extends AbstractQuadRenderer implements Consumer<Mesh> {
	...
	public void accept(Mesh mesh) {
		...
		while (index < limit) {
			Value mat = RenderMaterialImpl.byIndex(data[index]);
	...
	private void renderQuad(MutableQuadViewImpl q) {
		...
		final Value mat = q.material();
		final int textureCount = mat.spriteDepth();

		if (mat.hasAo && Minecraft.isAmbientOcclusionEnabled()) {
	 ...
	 */
	private void tesselateQuad(MutableQuadViewImpl quad, Value mat, int textureIndex) {
